...
int readFile(char *filename, char *Cbook, char *Traversal, char *Freq){
  File *fp;
  File *fp2;
  fp = fopen(Freq, "w");
 ...
 //fwrite(charct
  ...
  int elArrCt = 0;
    ...
    makeCode(elArr,freqArr, element, filename2 );
...
void makeCode(int *elArr, long *freqArr, int element, char *travFile, char *bookFile){
  ...
  int arr[10000], top =0;
  bookPrint(root, arr, top);
  output(root, arr, top, travFile, bookFile);
}
